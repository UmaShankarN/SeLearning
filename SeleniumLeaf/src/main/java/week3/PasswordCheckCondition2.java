...
public class PasswordCheckCondition2 {
	...
	public static void main(String[] args) 
	{
	 ...
	 int n=0,t=0,d=0;
	 if (passwordLength<10)//Length check
	 {
	  ...
	  if(passwordLength>=10)
	     ...
	     {
			 if(!((Arrays.toString(lower).contains(passwordArray[i]))||(Arrays.toString(capital).contains(passwordArray[i]))||(Arrays.toString(numbers).contains(passwordArray[i]))))
			 {
				n++;
			 }
		 } 
	// System.out.println(n);
	 ...
	 for(int i=0; i<passwordLength; i++)
	 {
	 if(((Arrays.toString(lower).contains(passwordArray[i]))||(Arrays.toString(capital).contains(passwordArray[i]))))
	 {
		 d++;
	 }
	 if(Arrays.toString(numbers).contains(passwordArray[i]))
			 {
		      t++;
			 }
	 }
	 ...
	 if(d<2||t<2)
	 {
		 System.out.println("Your password should contain atleast two letters and two digits");
	 }
